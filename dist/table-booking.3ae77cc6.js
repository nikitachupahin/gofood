...
})({"aQhdU":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "6681dc123ae77cc6";
"use strict";
...
},{}],"43yfF":[function(require,module,exports) {
document.addEventListener("DOMContentLoaded", function() {
    ...
    form.addEventListener("submit", async function(e) {
        ...
        try {
            ...
            if (isAvailable) {
                ...
                form.reset();
            } else alert("Sorry, the selected time is already booked. Please choose another time.");
        } catch (error) {
    ...
    async function checkAvailability(data) {
        ...
        for (let reservation of reservations){
            if (data.timeStart >= reservation.timeStart && data.timeStart < reservation.timeEnd || data.timeEnd > reservation.timeStart && data.timeEnd <= reservation.timeEnd || data.timeStart <= reservation.timeStart && data.timeEnd >= reservation.timeEnd) return false;
        }
        return true;
    }
    ...
    function updateAvailableTimeSlots(reservations) {
        ...
        [
            ...startTimeSelect.options
        ].forEach((option)=>option.disabled = false);
        [
            ...endTimeSelect.options
        ].forEach((option)=>option.disabled = false);
        const filteredReservations = reservations.filter((reservation)=>reservation.date === selectedDate && reservation.location === selectedLocation);
        filteredReservations.forEach((reservation)=>{
            [
                ...startTimeSelect.options
            ].forEach((option)=>{
                if (option.value >= reservation.timeStart && option.value < reservation.timeEnd) option.disabled = true;
            });
            [
                ...endTimeSelect.options
            ].forEach((option)=>{
...
},{}]},["aQhdU","43yfF"], "43yfF", "parcelRequireb934")

//# sourceMappingURL=table-booking.3ae77cc6.js.map
